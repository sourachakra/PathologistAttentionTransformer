...
def cc(s_map, gt, eps=1e-6):
    # Normalize the input maps
    s_map_norm = (s_map - torch.mean(s_map)) / (torch.std(s_map) + eps)
    gt_norm = (gt - torch.mean(gt)) / (torch.std(gt) + eps)
    ...
    denominator = torch.sqrt(torch.sum(a * a) * torch.sum(b * b)) + eps  # Add eps for stability
...
def focal_loss(pred, gt, alpha=2, beta=4, weights=None):
    ...
    #print('numel:',torch.numel(gt[gt==1]))
    neg_weights = torch.pow(1 - gt, beta)
    loss = 0
    #print('pos_inds:',pos_inds)
    #print('focal loss:',pred.size(),pos_inds.size(),alpha,weights.size())
    ...
    #print('numPos:',num_pos,pos_loss,neg_loss)
