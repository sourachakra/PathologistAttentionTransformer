...
def pos_to_action(center_x, center_y, patch_size, patch_num):
    ...
    #print('pos2ac:',x,y,patch_num[1],int(patch_num[1] * x + y))
    return int(patch_num[1] * (x-1) + (y))


def action_to_pos(acts, patch_size, patch_num):
    patch_x = acts // patch_num[1]
    patch_y = acts % patch_num[1]

    pixel_x = (patch_x + 1)* 1 + 1/2
    pixel_y = patch_y * 1 + 1/2  #patch_size[1]
...
def preprocess_fixations(trajs,
    ...
    mag_vals = [1,2,4,10,20,40]
    
    has_spid = 'scanpath_id' in trajs[0].keys()
    #print('ha spid:',has_spid)
    #print('traj len:',len(trajs))
    sum_traj_len = 0
    tran_all,tran_change,tran_increase = np.zeros((1,6)),np.zeros((1,6)),np.zeros((1,6))
    for traj in trajs:
        #print('orig traj:',traj['M'])
        if 5==5: #'7315' in traj['name']:
            #print('orig M:',traj['M'],len(traj['M']))
            if is_coco_dataset:  # For COCO-Search/Freeview datasets, force the initial fixation to be the center
                traj['X'][0], traj['Y'][0] = 20, 20 #traj['height']//2, traj['width']//2
            

            discrete_label = pos_to_action(traj['X'][0], traj['Y'][0], patch_size, patch_num)

            label = pos_to_action(traj['X'][0], traj['Y'][0], [1, 1], [traj['height'], traj['width']])
            fixs = [(traj['X'][0], traj['Y'][0])]
            mags = [mag_vals.index(traj['M'][0])]
            label_his = [discrete_label]
            #print('truncatenum:',truncate_num)
            if truncate_num < 1:
                traj_len = len(traj['X'])
            else:
                traj_len = min(truncate_num, len(traj['X']))

            sum_traj_len += traj_len
            
            other_wsis = []
            for traj1 in trajs:
                if traj1['name'] != traj['name'] and traj1['subject'] == traj['subject']:
                    #print(traj1['name'],traj1['subject'],traj['name'],traj['subject'])
                    other_wsis.append(traj1)
                
            for i in range(0, traj_len):
                discrete_label = pos_to_action(traj['X'][i], traj['Y'][i], patch_size, patch_num)

                #print('no discretize fix:')
                label = pos_to_action(traj['X'][i], traj['Y'][i], [1, 1], [traj['height'],traj['width']])
                ...
                curr_mag = mag_vals.index(traj['M'][i])

                
                tran_all[0,mags[-1]] += 1
                if mags[-1] != curr_mag:
                    tran_change[0,mags[-1]] += 1
                    if mags[-1] < curr_mag:
                        tran_increase[0,mags[-1]] += 1
                
                fix_label = [traj['name'], traj['condition'], copy(fixs), label, curr_mag, False, traj['subject'],
                                 np.sum(traj['T'][:i]),copy(mags),traj['height'],traj['width'],other_wsis]

                fixs.append((traj['X'][i], traj['Y'][i]))
                mags.append(mag_vals.index(traj['M'][i]))
            ...
            # if has_stop or sample_scanpath:
                ...
                # stop_label = [traj['name'], traj['condition'],copy(fixs), patch_num[0] * patch_num[1], curr_mag, True,
                                  # traj['subject'], np.sum(traj['T']),copy(mags),traj['height'],traj['width']]
                # fix_labels.append(stop_label)
    
        #print('mags_both:',mags,curr_mag,traj_len)
    #for k1 in range(len(fix_labels)):
        #print(fix_labels[k1][8],fix_labels[k1][4])
        
    print("\n===============\n")
    print(tran_change[0],tran_all[0],tran_increase[0])
    print((tran_change/tran_all)[0])
    print((tran_increase/tran_change)[0])
    print('fix lab:',len(fix_labels),'sum traj len:',sum_traj_len)
...
def get_prior_maps(gt_scanpaths, im_w, im_h, visual_angle=24):
    ...
    prior_maps = {}
    #for task in task_names:
    #traj = list(traj.values())
    #print('gt scans:',gt_scanpaths)
    gt_scanpaths = list(gt_scanpaths.values())
    Xs = np.concatenate([
    ...
    all_fixs = fixs #np.concatenate(all_fixs)
...
# def pos_to_action(center_x, center_y, patch_size, patch_num):
    # x = center_x // patch_size[0]
    # y = center_y // patch_size[1]
    ##print('pos2ac:',x,y,int(patch_num[1] * x + y))
    # return int(patch_num[1] * (x-1) + y)
...
# def action_to_pos(acts, patch_size, patch_num):
    ...
    # pixel_x = patch_x * 1 #+ 1/2
    # pixel_y = patch_y * 1 #+ 1/2  #patch_size[1]
    # return pixel_x, pixel_y
    
def transform_fixations(normalized_fixations, is_padding, pa, lr, hr, sample_scanpath=True, return_highres=False):
    ...
    def transform(normalized_fixations, is_padding, patch_num, sample_scanpath):
        ...
        labels = patch_num[1] * (fixs[:, :, 0]-1) + fixs[:, :, 1]
        labels[labels < 0] = 0
        labels[labels > pa.im_w*pa.im_h] = pa.im_h*pa.im_w-1
    ...
    fix_seq = transform(normalized_fixations, is_padding, [lr[1], lr[2]], sample_scanpath)
    if return_highres:
        fix_seq_high = transform(normalized_fixations, is_padding, [hr[1], hr[2]], sample_scanpath)
        #fix_seq_mid = transform(normalized_fixations, is_padding, [mr[1], mr[2]], sample_scanpath)
        return fix_seq, fix_seq_high #fix_seq_mid, 
    else:
        return fix_seq, None
    
def transform_fixations3(normalized_fixations, is_padding, pa, lr, mr, hr, sample_scanpath=True, return_highres=False):
